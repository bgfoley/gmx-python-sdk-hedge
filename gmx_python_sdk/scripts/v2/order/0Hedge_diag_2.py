...
from gmx_python_sdk.scripts.v2.order.order_argument_parser_2 import OrderArgumentParser2
...
def log_time(event):
    logger.info(f"{event}: {time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime())}")
...
log_time("Start creating connection")
connection = create_connection(config)
if connection is None:
    logger.error("Failed to establish a connection.")
    sys.exit(1)
log_time("Connection created")
...
log_time("Start estimating execution fee")
...
log_time("Execution fee estimated")
...
order_parser = OrderArgumentParser2(config, is_increase=True)
